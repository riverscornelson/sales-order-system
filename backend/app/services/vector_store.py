...
class VectorStore:
    """Vector storage and search service using Google Cloud Storage"""
    
    def __init__(self):
        self.project_id = os.getenv('GOOGLE_CLOUD_PROJECT')
        self.bucket_name = os.getenv('GCS_PARTS_CATALOG_BUCKET')
        
        if not self.project_id or not self.bucket_name:
            raise ValueError("GOOGLE_CLOUD_PROJECT and GCS_PARTS_CATALOG_BUCKET required")
        ...
        logger.info("Initialized vector store", bucket=self.bucket_name)
    
    async def store_vectors(self, vectors: List[Dict[str, Any]], 
                           collection_name: str = "parts_catalog") -> bool:
        """Store vectors with metadata to GCS"""
        ...
        try:
            ...
            # Store to GCS
            blob_name = f"vectors/{collection_name}.json"
            blob = self.bucket.blob(blob_name)
            
            blob.upload_from_string(
                json.dumps(vector_data, default=str),
            ...
            logger.info("Stored vectors to GCS", 
    ...
    async def load_vectors(self, collection_name: str = "parts_catalog") -> List[Dict[str, Any]]:
        """Load vectors from GCS with caching"""
        ...
        try:
            ...
            logger.info("Loaded vectors from GCS", 
                       collection=collection_name,
                       count=len(vectors))
    ...
    async def delete_vector(self, vector_id: str, 
        ...
        try:
            ...
            updated_vectors = [v for v in vectors if v.get("id") != vector_id]
            
            if len(updated_vectors) == len(vectors):
                logger.warning("Vector not found for deletion", id=vector_id)
                return False
            
            # Store updated collection
            success = await self.store_vectors(updated_vectors, collection_name)
            
            if success:
                logger.info("Deleted vector", id=vector_id, collection=collection_name)
    ...
    async def list_collections(self) -> List[str]:
        ...
        try:
            blobs = self.bucket.list_blobs(prefix="vectors/")
            collections = []
            
            for blob in blobs:
                if blob.name.endswith('.json'):
                    # Extract collection name from blob path
                    collection_name = blob.name.replace('vectors/', '').replace('.json', '')
                    collections.append(collection_name)
            
            logger.info("Listed vector collections", count=len(collections))
            return collections
...
class PartsCatalogVectorStore(VectorStore):
    """Specialized vector store for parts catalog"""
